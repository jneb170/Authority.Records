...
public sealed class CountMapMarkersHandlerTests
{
    [Fact]
    public async Task Handle_CountsOnlyAgencyIncidents()
    {
        await using var db = CountMarkersTestDbContext.Create();
        ...
        var handler = new CountMapMarkersHandler(db, new FakeTenantProvider(jurisdictionId, agencyId));

        var count = await handler.Handle(new CountMapMarkersQuery(jurisdictionId, null), CancellationToken.None);

        Assert.Equal(1, count);
    }

    [Fact]
    public async Task Handle_CountsOnlyAgencyArrests()
    {
        await using var db = CountMarkersTestDbContext.Create();
        var jurisdictionId = Guid.NewGuid();
        var agencyId       = Guid.NewGuid();
    ...
    [Fact]
    public async Task Handle_CountsOnlyAgencyCitations()
    {
        await using var db = CountMarkersTestDbContext.Create();
        var jurisdictionId = Guid.NewGuid();
        var agencyId       = Guid.NewGuid();
    ...
    [Fact]
    public async Task Handle_ReturnsZero_WhenAgencyIdIsEmpty()
    {
        await using var db = CountMarkersTestDbContext.Create();
        var jurisdictionId = Guid.NewGuid();
        var locationId     = Guid.NewGuid();
        ...
        var handler = new CountMapMarkersHandler(db, new FakeTenantProvider(jurisdictionId, Guid.Empty));
    ...
    [Fact]
    public async Task Handle_ReturnsSumAcrossAllRecordTypes_ForAgency()
    {
        await using var db = CountMarkersTestDbContext.Create();
        var jurisdictionId = Guid.NewGuid();
        var agencyId       = Guid.NewGuid();
        ...
        // Two agency records of each type + one non-agency record of each type
        db.IncidentReadModels.Add(CreateIncident(Guid.NewGuid(), 1001, jurisdictionId, agencyId,    locationId));
        db.IncidentReadModels.Add(CreateIncident(Guid.NewGuid(), 1002, jurisdictionId, agencyId,    locationId));
        db.IncidentReadModels.Add(CreateIncident(Guid.NewGuid(), 9001, jurisdictionId, otherAgencyId, locationId));
        ...
        // 2 incidents + 1 arrest + 1 citation = 4
        Assert.Equal(4, count);
    }

    [Fact]
    public async Task Handle_ReturnsZero_WhenNoLocationsHaveCoordinates()
    {
        await using var db = CountMarkersTestDbContext.Create();
        var jurisdictionId = Guid.NewGuid();
        var agencyId       = Guid.NewGuid();
        ...
        db.LocationReadModels.Add(CreateLocation(locationId, jurisdictionId, null));
        db.IncidentReadModels.Add(CreateIncident(Guid.NewGuid(), 1001, jurisdictionId, agencyId, locationId));
        await db.SaveChangesAsync();
    ...
    private sealed class CountMarkersTestDbContext(DbContextOptions<CountMarkersTestDbContext> options)
    ...
    public DbSet<NarrativeLinkReadModel> NarrativeLinkReadModels => throw new NotImplementedException();
        ...
        public static CountMarkersTestDbContext Create()
        {
            var options = new DbContextOptionsBuilder<CountMarkersTestDbContext>()
                .UseInMemoryDatabase(Guid.NewGuid().ToString())
                .Options;
            return new CountMarkersTestDbContext(options);
        }
    }
}
