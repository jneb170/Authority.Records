    ...
    INotificationHandler<MugshotUnlinkedFromOwnerDomainEvent>
{
    ...
    public async Task Handle(MugshotLinkedToOwnerDomainEvent notification, CancellationToken cancellationToken)
    {
        ...
        await UpdateOwnerPreviewAsync(notification.OwnerType, notification.OwnerId, cancellationToken);
    }
    ...
    private async Task UpdateOwnerPreviewAsync(string ownerType, Guid ownerId, CancellationToken cancellationToken)
    {
        var primaryLink = await _dbContext.MugshotLinkReadModels
            .AsNoTracking()
            .Where(l => l.OwnerType == ownerType && l.OwnerId == ownerId)
        ...
        if (primaryLink is not null)
        {
            primaryUrl = await _dbContext.MugshotReadModels
                .AsNoTracking()
                .Where(m => m.Id == primaryLink.MugshotId)
